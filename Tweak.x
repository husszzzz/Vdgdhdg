...
// ==========================================
// 2. تصميم المنيو، التوهج، والأزرار
// ==========================================
%hook GBModMenu

// إضافة وظيفة لزر "قناتنا" تفتح الرابط الخاص بيك
%new
...
- (void)layoutSubviews {
    ...
    // دالة تلف على كل العناصر داخل المنيو وتعدلها
    void (^__block processViews)(UIView *) = ^(UIView *view) {
        ...
        // تعديل الأزرار (UIButton)
            ...
            // تعديل زر قناتنا
                ...
                [btn addTarget:self action:@selector(openHasanyChannel:) forControlEvents:UIControlEventTouchUpInside];
            }
            // تعديل زر المطور
                ...
                [btn removeTarget:nil action:NULL forControlEvents:UIControlEventAllEvents];
                [btn addTarget:self action:@selector(openHasanyDev:) forControlEvents:UIControlEventTouchUpInside];
            }
            // زر إعادة الإعدادات
    ...
    // تشغيل الدالة على المنيو
    processViews(mainMenu);
}
